...
**Note:** _recent releases are `ESM`-only. If you need `CommonJS` support you should use
`v0.10`. You can use the latest version (`ESM`) in `Next.js`, just make sure
your next config file is `ESM`: `next.config.mjs`._<br /> _Here's an example:
[github.com/atomiks/rehype-pretty-code/website/next.config.mjs](https://github.com/atomiks/rehype-pretty-code/blob/master/website/next.config.mjs)_

```sh
npm install rehype-pretty-code@0.10.0
```
...
const options = {
  theme: JSON.parse(
    readFileSync(new URL('./themes/my-theme.json', import.meta.url))
  ),
};
```
