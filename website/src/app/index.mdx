...
To customize the theme, background color, and highlighting, options can be
specified.
...
This behavior is enabled by specifying the style (e.g. className or data
attribute) via the visitor callback hooks
(`onVisitHighlightedLine{:.entity.name.function}`,
`onVisitHighlightedWord{:.entity.name.function}`).
