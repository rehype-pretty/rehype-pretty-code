[Rehype Pretty Code](https://github.com/atomiks/rehype-pretty-code) is a
build-time (or server-side) Rehype plugin that provides beautiful code blocks
for Markdown or MDX.

It's fast since it avoids runtime syntax highlighting, and works with new
features like React Server Components.
...
Code blocks are unstyled by default to give you full control. Code blocks
however have a `display: grid{:sass}` style by default to allow line
highlighting to span the entire width of a horizontally scrollable code block.
...
But you can use your own JSON theme as well by passing the theme JSON:
