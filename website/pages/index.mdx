...
onVisitHighlightedWord(node, id) {
  // `id` will be either 'a' or 'b'
  const color = {a: 'red', b: 'blue'}[id];
  node.properties.style = `color: ${color}`;
}
```
...
````md
```js showLineNumbers
// <code> will have a `data-line-numbers` attribute and CSS variable `--line-numbers-length`
```
````
